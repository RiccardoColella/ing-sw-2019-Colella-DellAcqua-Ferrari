...
public class WeaponFactory {
     ...
     */
    private static Map<String, JsonObject> weaponMap;
    ...
    private static void readFromFile() {
        ...
        jsonElement.getAsJsonArray().forEach(weaponJson -> {
            ...
            weaponMap.put(
                    weaponObject.get(Property.WEAPON_ID.toString()).getAsString(),
                    weaponObject
            );
        });
    }
    ...
    private static Weapon readWeapon(JsonObject weaponObject, Board board) {
        ...
        if (weaponObject.has(Property.ALTERNATIVE_ATTACK.toString())) {
            ...
            weaponObject.get(Property.ADVANCED_ATTACKS.toString()).getAsJsonArray().forEach(attack -> {
                ...
                boolean basicFirst = attack.getAsJsonObject().has(Property.BASIC_MUST_BE_FIRST.toString()) && attack.getAsJsonObject().get(Property.BASIC_MUST_BE_FIRST.toString()).getAsBoolean();
     ...
     * see {@code jsonAttacks.md} for further info
     ...
     * the {@code JsonObject} to have a property named {@code targets}, see {@code jsonAttacks.md} for further info
     ...
     * has the property {@code targetPosition}, see {@code jsonAttacks.md} for further info
     * @param actionObject the {@code JsonObject} to analyze
     ...
     * supported values, see {@code jsonAttacks.md} for further info
     * @param actionObject the {@code JsonObject} to analyze
     * @param board the {@code Board} on which the targets will be found
     * @param lastTargetCalculator the previous {@code TargetCalculator} for this attack, which can be {@code null}
     * @return the new {@code TargetCalculator}
     ...
     * This property must exist, see {@code jsonAttacks.md} for further info about the supported values
     * @param actionObject the {@code JsonObject} to analyze
    ...
    /**
     * This method reads the {@code startingPoint} property of {@code actionObject}. See {@code jsonAttacks.md} for info
    ...
    /**
     * This method reads the type of the action, possible values are specified in {@code jsonAttacks.md}
     *
     * @param actionObject the {@code JsonObject} to analyze
