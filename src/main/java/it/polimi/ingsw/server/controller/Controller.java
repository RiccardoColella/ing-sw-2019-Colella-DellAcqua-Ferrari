...
public class Controller implements Runnable, PlayerDamagedListener {
    ...
    private void grabOnASpawnpoint(Player activePlayer, Interviewer view){
        ...
        List<Weapon> affordableWeapons = availableWeapons.stream().filter(weapon -> PaymentHandler.canAfford(weapon.getAcquisitionCost(), activePlayer))
    ...
    private void executeShoot(Player activePlayer, Interviewer view){
        Optional<Weapon> activeWeapon = activePlayer.getActiveWeapon();
        if (activeWeapon.isPresent()){
            Weapon weapon = activeWeapon.get();
            if (weapon.isLoaded()) {
    ...
    private void executeReload(Player activePlayer, Interviewer view){
        List<Weapon> playerWeapons = activePlayer.getWeapons();
        List<Weapon> weaponsToReload = new ArrayList<>(playerWeapons);
        List<Weapon> weaponsReloadable = weaponsToReload
                .stream()
        ...
        if (weaponsReloadable.isEmpty()){
            ...
            Weapon weaponToReload = view.select("Which weapon would you like to reload?", weaponsReloadable, ClientApi.RELOAD_QUESTION);
     ...
     */
    private void reloadWeapon(Weapon weapon, Player activePlayer, Interviewer view){
        ...
        switch (action){
            ...
            case GRAB:
                if (player.isOnASpawnpoint()) {
                    ...
                    List<Weapon> availableWeapons = new LinkedList<>(block.getWeapons());
                    List<Weapon> affordableWeapons = availableWeapons.stream().filter(weapon -> PaymentHandler.canAfford(weapon.getAcquisitionCost(), player))
                            .collect(Collectors.toList());
